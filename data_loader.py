...
def collate_fn(data):#对DataLoader产生的Tensorlist转为更高维度的Tensor
    bert_inputs, grid_labels, grid_soft_labels,grid_mask2d, pieces2word, dist_inputs, sent_length, entity_text = map(list, zip(*data))
    ...
    soft_labels_mat = torch.zeros((batch_size, max_tok, max_tok, grid_soft_labels[0].shape[-1]), dtype=torch.float)
    grid_soft_labels = fill_3d(grid_soft_labels, soft_labels_mat)
    ...
    return bert_inputs, grid_labels,grid_soft_labels , grid_mask2d, pieces2word, dist_inputs, sent_length, entity_text


class RelationDataset(Dataset):
    def __init__(self, bert_inputs, grid_labels, grid_soft_labels, grid_mask2d, pieces2word, dist_inputs, sent_length, entity_text):
        self.bert_inputs = bert_inputs
        self.grid_labels = grid_labels
        self.grid_soft_labels = grid_soft_labels
    ...
    def __getitem__(self, item):#经过DataLoader调用，每个list的每个元素从numpy转为Tensor
        return torch.LongTensor(self.bert_inputs[item]), \
               torch.LongTensor(self.grid_labels[item]), \
            torch.FloatTensor(self.grid_soft_labels[item]), \
...
def data_filter(data):
    ...
    for index, instance in enumerate(data):
        if len(instance['sentence']) == 0 or len(instance['sentence']) > length_limit:
            continue
        results.append(instance)
    return results
def process_bert(data, tokenizer, vocab, is_trainset = False):
    ...
    for index, instance in enumerate(data):
        ...
        for entity in instance["ner"]:
            index = entity["index"]
            _grid_labels[ index[0],index[-1]] = vocab.label_to_id(entity["type"])
...
def load_data_bert(config):
    ...
    dev_data = data_filter(dev_data)
    test_data = data_filter(test_data)
    ...
    train_dataset = RelationDataset(*process_bert(train_data, tokenizer, vocab , True))
